...
dmatrix bestGlobalPosition(1,Dim);  // 全ての群れの中で最適な粒子の位置
...
void Particles::initializeParticles(){
  ...
  for(int i=0; i<numberParticle; i++){
       ...
       // ----- 初期値代入 ----- //
       row(position,i)=row(randomPosition,0);
       fitness(0,i)=fit;
       row(velocity,i)=row(randomVelocity,0);
       row(bestPosition,i)=row(randomPosition,0);
       bestFitness(0,i)=fit;

       // --- Particleオブジェクトの適合度がこれまでで最適な適合度 --- //
       if(fit<bestGlobalFitness){
           bestGlobalFitness=fit;
           row(bestGlobalPosition,0)=row(position,i);
       }
  }
...
int main()
{
  ...
  for(int iteration=1; iteration<numberIterations; iteration++) {
      for(int i=0; i<numberParticle; i++) {
          ...
          // Particleオブジェクトの速度更新
          row(p.velocity,i)=row(newVelocity,0);
          ...
          // Particleオブジェクトの位置更新
          row(p.position,i)=row(newPosition,0);
          ...
          if(newFitness<p.bestFitness(0,i)) {
              row(p.bestPosition,i)=row(newPosition,0);
