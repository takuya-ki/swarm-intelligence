...
int main()
{
  ...
  for(int iteration=1; iteration<=timeSteps; iteration++) {
      // step1: 前時刻の各粒子の最適解を現時刻の評価関数で再評価
      // 現時刻の評価関数における最良解に最も近いpbest
      for(int i=0; i<numberParticle; i++) {
        tmpPosition = row(p.bestPosition,i);
        p.bestFitness(0,i)=calObjFunc(tmpPosition,iteration);
      }
      vector<double> g1 = row(p.bestFitness,0);
      vector<double>::iterator ite1 = min_element(g1.begin(),g1.end());
      int i1 = distance(g1.begin(),ite1);
      bestGlobalPosition=row(p.bestPosition,i1);
      ...
      for(int i=0; i<numberParticle; i++) {
          ...
          newFitness=calObjFunc(newPosition,iteration);  // 新しい適合度計算
          p.fitness(0,i)=newFitness;           // Particleオブジェクトの適合度更新

          // step3:pbest更新
          if(newFitness<p.bestFitness(0,i)) {
              row(p.bestPosition,i)=newPosition;
              p.bestFitness(0,i)=newFitness;
          }
      }

      // step4:gbest更新
      vector<double> g4 = row(p.bestFitness,0);
      vector<double>::iterator ite4 = min_element(g4.begin(),g4.end());
      int i4 = distance(g4.begin(),ite4);
      bestGlobalPosition=row(p.bestPosition,i4);
      bestGlobalFitness = *ite4;
